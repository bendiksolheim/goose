...
func renderLog(log: AsyncData<LogInfo>) -> Node {
    ...
    case let .Success(log):
        ...
        let commits = log.commits.map(renderLogLine)
...
func renderLogLine(_ line: GitLogLine) -> Node {
    switch line {
    case let .CommitLine(commit):
        return renderCommit(commit)
    case let .GraphLine(graph):
        return Text("        " + graph)
    }
}

func renderCommit(_ commit: GitCommit) -> Node {
    let graph = Text(commit.graph.fold({ "" }, { " \($0)"} ))
...
func formatCommitDate(_ date: Date) -> String {
    ...
    return "\(String(value).leftPad(toLength: 2, withPad: " ")) \(value > 1 ? units : unit)"
}